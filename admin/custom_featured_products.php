...
$featured_file = $_SERVER['DOCUMENT_ROOT'].'/admin/custom_featured_products_list.txt';
...
if(isset($_POST['search'])){
	...
	$_POST['search'] = $mysqli->real_escape_string($_POST['search']);
...
$(document).ready(function(){
	...
	$('#Content').on('click', '.Product a', function(){
		return false;
	})
	...
	$('#FeaturedOptions').on('click', '.SortItem', function(e){
		if(<?= $featured_product_limit ?> == 0 || $('.Content.Featured .SortItem').length < <?= $featured_product_limit ?>){
	...
	$('.FindProduct').donetyping(function() {
		...
		$.ajax({
			type: "POST",
			url: "custom_featured_products.php",
...
<div class="Details"><p>Drag products around to change display order.</p></div>
...
<p>Click on a product below to add it to the Featured Product list above.</p>
