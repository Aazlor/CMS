<?php
/** SubPage Edit Photogallery Submit **/
/** SubPage Edit Photogallery Submit **/
/** SubPage Edit Photogallery Submit **/
/** SubPage Edit Photogallery Submit **/
/** SubPage Edit Photogallery Submit **/
	...
	foreach($_POST as $key => $value){
		if(preg_match('/^Current{}/', $key)){
			...
			if($_POST['Remove{}'.$key] != 'yes'){
				...
				$record_images[$key] = '';
				unlink('../photogallery/'.$_POST[field_name].'/'.$value);
			}
		}
	}
	
	if(in_array('{file}', $photogallery_vars)){
		$key_chk = array_search('{file}', $photogallery_vars);
	}	
	foreach($_FILES as $key=>$ivalue){			
		if($key_chk == $key && $_FILES[$key]['name'] != ''){
			$source = $_FILES[$key]['tmp_name'];
			$target = "../photogallery/".$_POST[field_name]."/".basename( $_FILES[$key]['name']);
		...
		elseif($_FILES[$key]['name'] != ''){
			...
			$photogallery_Image_sizes = $photogallery_vars[$key.'{image}'];
			foreach($photogallery_Image_sizes as $image_size_key => $image_size_value){
				...
				if($photogallery_sizing[$file_key.'{image}']['width'] != '' || $template_vars[$file_key.'{image}']['height'] != ''){
	...
	require("../photogallery/$_POST[field_name]/".$_GET[y]."-images.php");	
	...
	$file = fopen('../photogallery/'.$_POST[field_name].'/'.$_GET[y].'-images.php', "w+");
	fwrite($file, $writearray);
	fclose($file);	

	echo '<div class="Message"><img src="images/tick.gif"> '.$photogallery_vars['Type'].' updated.</div>';
