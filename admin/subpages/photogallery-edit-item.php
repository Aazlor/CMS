<?php
/** SubPage Manage/Edit Photogallery Item **/
/** SubPage Manage/Edit Photogallery Item **/
/** SubPage Manage/Edit Photogallery Item **/
/** SubPage Manage/Edit Photogallery Item **/
/** SubPage Manage/Edit Photogallery Item **/
	...
	require("../photogallery/".$_GET['field_name']."/".$_GET['yy']."-images.php");	
		...
		foreach($vars as $kp => $vp){
			if(preg_match('{photogallery}', $vp)){
				...
				foreach($image_array as $key => $value){
					...
					$listitems .= '<li id="listItem_'.$key.'"><img src="images/arrow.png" alt="move" width="16" height="16" class="handle" /><img src="../photogallery/'.$split[0].'"  width="75"></li>';
					$listorder .= '<div style="font-size: 14px; font-family: arial; padding-bottom: 3px; float: left; width: 170px; margin: 10px;"><img src="../photogallery/'.$_GET['yy'].'-'.$split[0].'" width="150"></div>';
					$a++;
				}
				
				$split = explode('||', $image_array[$_GET[id]]);
						...
						if($split[$i] == ''){
							...
							<div class="FieldInput LabelInsert">
								...
								<div class="InlineBlock">
									<img src="/photogallery/'.$field_name.'/'.$split[$i].'">
								</div>
								<div class="InlineBlock">
					...
					elseif(preg_match('/{file}/', $value)){
						if($split[$i] == ''){
							...
							<div class="FieldInput LabelInsert">
								...
								<div class="InlineBlock">
									Current File: <a href="/photogallery/'.$field_name.'/'.$split[$i].'" target="_blank">http://www.'.$_SERVER['SERVER_NAME'].'/photogallery/'.$field_name.'/'.$split[$i].'</a>
								</div>
				...
				echo'		
					<div class="Submit">
						...
						<input type="hidden" name="submit_edit_photogallery" value="yes"/>
						<input type="submit" value="Edit '.$photogallery_vars[Type].'" />
					</div>
				</form>
				';
